<template>
  <div class="main">
    <div id="background"></div>
    <div id="timing">
<<<<<<< HEAD
      <span id="info">Time: {{ time }}<br>Current: {{ info.currentName }}, Next: {{ info.nextName }}</span><br>
      <span id="timeleft">{{ info.currentTimeLeft }}</span>
    </div>
    
    <!-- <div id="settings">
    </div> -->

    <router-link to="/settings" id="settings">
      <img v-if="$store.getters.contrastColor == 'black'" src="../assets/settings-black-18dp.svg" id="settings-img"> 
      <img v-else src="../assets/settings-white-18dp.svg" id="settings-img"> 
    </router-link>
=======
      <span id="info">Current: {{ info.currentName }}, Next: {{ info.nextName }}</span>
      <br />
      <span id="timeleft">{{ info.currentTimeLeft }}</span>
    </div>
    <OauthLogin clientID="408077029007-b2j0stflkuace7jiquvc8glo17g9gir9" />
>>>>>>> 5f69ea0c67db697f4c36e05d85e04ad003fc99f6
  </div>
</template>
<!-- <HelloWorld msg="Welcome to Your Vue.js + TypeScript App"/> -->

<script lang="ts">
import { Options, Vue } from "vue-class-component";
import store from "../store/index";
import { state, mutations, getters, actions } from "../store/store";
import { ExternalSchedualLoader } from "chronos-time";
import OauthLogin from "../components/OauthLogin.vue";

// import HelloWorld from '@/components/HelloWorld.vue'
// import { mapMutations, mapState } from 'vuex'
// components: {
// HelloWorld,
// },
// computed: {
//   ...mapState(["backgroundColors"])
// }

@Options({ name: "Main", components: { OauthLogin } })
export default class Main extends Vue {
<<<<<<< HEAD
  $store!: Exclude<typeof store, "state" | "mutations" | "actions" | "getters"> & {getters: typeof getters} & {state: typeof state} & {mutations: typeof mutations} & {actions: typeof actions}
  schedule!: ExternalSchedualLoader
  interval!: number
  time = ""
  info: {
    currentTimeLeft?: ExternalSchedualLoader["currentTimeLeft"],
    currentTag?: ExternalSchedualLoader["currentTag"],
    currentName?: ExternalSchedualLoader["currentName"],
    nextName?: ExternalSchedualLoader["nextName"]
  } = {}
  alerted = false

  addZero(n: number) {
    return (n > 9 ? "" : "0") + n
  }

  public mounted() {
    console.log(this.$store.getters)
    // document.getElementById("settings-img")

    this.schedule = new ExternalSchedualLoader("https://chronoshhs.herokuapp.com/hhs", "https://chronoshhs.herokuapp.com/HHSTodayIs")
    this.interval = window.setInterval((() => {
      ({currentTimeLeft: this.info.currentTimeLeft, currentTag: this.info.currentTag, currentName: this.info.currentName, nextName: this.info.nextName} = this.schedule);
      this.info.currentTimeLeft = typeof this.info.currentTimeLeft != "string" ? (this.info.currentTimeLeft || [0,0,0]).map(this.addZero).join(":") : this.info.currentTimeLeft.replace("No schedule for", "Nothing")
      const now = new Date()
      this.time =  [now.getHours() % 12, now.getMinutes(), now.getSeconds()].map(this.addZero).join(":") + (now.getHours() < 12 || now.getHours() == 24 ? " am" : " pm")
      if(!this.alerted && typeof this.info.currentTimeLeft != "string" && ((this.info.currentTimeLeft[0] * 60 + this.info.currentTimeLeft[1]) + this.info.currentTimeLeft[2]) < 60) {
        this.alerted = true
        alert(":OOO less than 1 minute left")
      } else
        this.alerted = false
      document.title = this.info.currentTimeLeft
    }).bind(this), 1000)
=======
  $store!: typeof store & { state: typeof state } & {
    mutations: typeof mutations;
  } & { actions: typeof actions } & { getters: typeof getters };
  schedule!: ExternalSchedualLoader;
  interval!: number;
  info: {
    currentTimeLeft?: ExternalSchedualLoader["currentTimeLeft"];
    currentTag?: ExternalSchedualLoader["currentTag"];
    currentName?: ExternalSchedualLoader["currentName"];
    nextName?: ExternalSchedualLoader["nextName"];
  } = {};

  public mounted() {
    console.log(this.$store);
    this.schedule = new ExternalSchedualLoader(
      "https://chronoshhs.herokuapp.com/hhs",
      "https://chronoshhs.herokuapp.com/HHSTodayIs"
    );
    this.interval = window.setInterval(
      (() => {
        ({
          currentTimeLeft: this.info.currentTimeLeft,
          currentTag: this.info.currentTag,
          currentName: this.info.currentName,
          nextName: this.info.nextName
        } = this.schedule);
        this.info.currentTimeLeft =
          typeof this.info.currentTimeLeft != "string"
            ? (this.info.currentTimeLeft || [0, 0, 0])
                .map(n => (n < 10 ? "0" : "") + n)
                .join(":")
            : this.info.currentTimeLeft;
        document.title = this.info.currentTimeLeft;
      }).bind(this),
      1000
    );
>>>>>>> 5f69ea0c67db697f4c36e05d85e04ad003fc99f6
  }

  public beforeUnmount() {
    if (this.interval) window.clearInterval(this.interval);
  }
}
</script>

<style lang="scss" scoped>
<<<<<<< HEAD
  @keyframes AnimateBackground {
      0%{background-position:0% 50%}
      50%{background-position:100% 50%}
      100%{background-position:0% 50%}
=======
@keyframes AnimationName {
  0% {
    background-position: 0% 50%;
>>>>>>> 5f69ea0c67db697f4c36e05d85e04ad003fc99f6
  }
  50% {
    background-position: 100% 50%;
  }
  100% {
    background-position: 0% 50%;
  }
}

.main {
  --color: white;
  display: grid;
  grid-template: 1fr auto 1fr / 1fr auto 1fr;
  height: 100vh;
  width: 100vw;

<<<<<<< HEAD
    background-color: #8BC6EC;
    background-image: linear-gradient( 135deg,  rgba(48,207,208,1) 11.2%, rgba(51,8,103,1) 92.5% );
    //   background: 
    // radial-gradient(
    //   circle at top left,
    //   rgba(237, 237, 237, 0.5), 
    //   transparent 100px
    // ),
    // radial-gradient(
    //   circle at top right,
    //   rgba(209, 253, 255, 0.5), 
    //   transparent 100px
    // ),
    // radial-gradient(
    //   at bottom left,
    //   rgba(255, 0, 255, 0.5), 
    //   transparent 400px
    // ),
    // radial-gradient(
    //   at bottom right,
    //   rgba(255, 100, 100, 0.5), 
    //   transparent 400px
    // );
    
    background-size: 120%;
    animation: AnimateBackground 15s cubic-bezier(0.075, 0.82, 0.165, 1) infinite;

    #background {
      position: absolute;
      left: 0;
      top: 0;
      width: 100vw;
      height: 100vh;

      opacity: .75;

      background-image: url(data:image/png;base64,);
      animation: AnimateBackground 15s cubic-bezier(0.075, 0.82, 0.165, 1) infinite;
    }

    #timing {
      z-index: 1;
      grid-area: 2 / 2;
=======
  background-color: #8bc6ec;
  background-image: linear-gradient(
      109.6deg,
      rgba(48, 207, 208, 1) 11.2%,
      rgba(51, 8, 103, 1) 92.5%
    ),
    url(data:image/png;base64,);
  background-size: 120%;
  animation: AnimationName 15s cubic-bezier(0.075, 0.82, 0.165, 1) infinite;

  #timing {
    grid-area: 2 / 2;
>>>>>>> 5f69ea0c67db697f4c36e05d85e04ad003fc99f6

    #info {
      color: var(--color);

<<<<<<< HEAD
        float: left;
        font-style: italic;
        font-size: 1.25rem;
        text-align: left;
=======
      float: left;
      font-style: italic;
      font-size: 1.25rem;
>>>>>>> 5f69ea0c67db697f4c36e05d85e04ad003fc99f6

      position: relative;
      top: 15px;
      left: 10px;
    }

<<<<<<< HEAD
      #timeleft {
        color: var(--color);
        text-align: left;
        float:left;
=======
    #timeleft {
      color: var(--color);
>>>>>>> 5f69ea0c67db697f4c36e05d85e04ad003fc99f6

      position: relative;
      bottom: 15px;
      box-sizing: content-box;
      font-size: 7.5rem;
    }

    #settings {
      position: absolute;
      right: 40px;
      bottom: 40px;

      width: auto;
      height: auto;

      transition: transform .3s ease-in-out;

      &:hover {
        transform: scale(1.3);
      }

      #settings-img {
        width: 40px;
        height: 40px;
        animation: pulse 6s infinite;
      }
    }

    @keyframes pulse {
      0% {
        transform: rotateZ(0);
      }
      70% {
        transform: rotateZ(360deg);
      }
    }

  }
}
</style>